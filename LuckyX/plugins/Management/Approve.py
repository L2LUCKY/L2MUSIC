from LuckyX import app as Rani
...
@Rani.on_callback_query(filters.regex("close"))
...
@Rani.on_callback_query(filters.regex("gib_source"))
...
bg_path = "LuckyX/assets/adipic.png"
font_path = "LuckyX/assets/Rani.ttf"

# --------------------------------------------------------------------------------- #

# Extract environment variables or provide default values
chat_id_env = environ.get("CHAT_ID")
CHAT_ID = [int(Rani) for Rani in chat_id_env.split(",")] if chat_id_env else []
...
@Rani.on_chat_join_request((filters.group | filters.channel) & filters.chat(CHAT_ID) if CHAT_ID else (filters.group | filters.channel))
async def autoapprove(client: Rani, message: ChatJoinRequest):
    chat = message.chat  # Chat
    user = message.from_user  # User
    photo = await Rani.download_media(user.photo.big_file_id)
